...
/**
 * CRUD de patrones bajo /api/v1. Lectura pública; escritura solo Super Admin/Admin.
...
public class PatronResource {
    ...
    public Response crear(CrearCatalogoRequest req) {
        return Response.status(Response.Status.CREATED).entity(patronService.crear(req)).build();
    }

    @PUT
    @Path("/{id}")
    public PatronDto actualizar(@PathParam("id") Long id, ActualizarCatalogoRequest req) {
        return patronService.actualizar(id, req);
    }

    @DELETE
    @Path("/{id}")
    public MensajeResponse eliminar(@PathParam("id") Long id) {
        return patronService.eliminar(id);
    }
}