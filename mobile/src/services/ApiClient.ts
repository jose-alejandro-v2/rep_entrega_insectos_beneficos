 ...
 * SIEMPRE usa puerto 6113 y base path `/api/v1`.
 *
 * Reglas (en orden): trim + quita barras finales; sin esquema → antepone
 * `http://` (soporte https conservado para producción futura); si no hay puerto
 * explícito se usa 6113; sin `/api/v1` → lo añade al
 * final. Idempotente: URLs ya completas (`http://host:6113/api/v1`) no se
 ...
 *   'http://miservidor:6113/api/v1'  → sin cambios
 */
export function normalizeApiUrl(url: string): string {
  ...
  // Si el usuario no especifica puerto, usar 6113 por defecto.
  const hostSinPuerto = hostPort.includes(':') ? hostPort.slice(0, hostPort.indexOf(':')) : hostPort;
  const puerto = hostPort.includes(':') ? hostPort.slice(hostPort.indexOf(':') + 1) : '6113';
  out = `${scheme}${hostSinPuerto}:${puerto}${rest}`;
  if (!/\/api\/v1(\/|$)/.test(out)) {
